...
Describe "Convert-ShortcutTargetPath関数をテストする" {
    ...
    It "ひとつのショートカットのtargetPathを書きかえる操作をDryrunする --- 書き換えた結果のディレクトリが存在する場合" {
        ...
        (Convert-ShortcutTargetPath -Shortcut $shrt -Regexp 'AppData$' -Replacement 'AppData\Roaming\Google\Chrome' -Dryrun) | Should Be $true
    }
    
    It "ひとつのショートカットのtargetPathを書きかえる操作をDryrunする --- 書き換えた結果のディレクトリが存在しない場合" {
        $shrt = Join-Path $workDir "subfolder\moreLinkToAppData.lnk"
        (Convert-ShortcutTargetPath -Shortcut $shrt -Regexp 'AppData$' -Replacement 'AppData\Roaming\Google\FOOBAR' -Dryrun) | Should Be $false
    }
